...
site() {
  ...
  case "$CMD" in
    deploy)
      [ -z "$3" ] && echo "Usage: site deploy <dir> <project>" && return 1
      echo "🚀 Deploying '$3'..."
      rclone sync "$2" "$REMOTE/$3/" --progress --transfers 16 --checksum --delete-excluded
      local URL="https://${3}-site.tamatar.dev"
      _copy_to_clip "$URL"
      echo "🌍 Live at: $URL"
      ;;
    ls) echo "📂 Active Projects:"; rclone lsf "$REMOTE" --dirs-only ;;
    rm) rclone purge "$REMOTE/$2/"; echo "🔥 Deleted project '$2'" ;;
